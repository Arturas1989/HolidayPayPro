...
export function getAllWorkDays(year: string | undefined){
  ...
  if(year){
    ...
    for(let month = 10; month <= 12; month++){
      allWorkDays[`${currYear}/${month}`] = getWorkDays('' + currYear, month);
    }
    
    for(let month = 1; month <= 12; month++){
      allWorkDays[`${year}/${month}`] = getWorkDays(year, month);
    }
  }
  return allWorkDays
...
export function getWorkDays(year: string | undefined, monthNum: number){
  if(year){

    const lastDay = getLastDay(+year, monthNum);
    const lastDayNum = lastDay.getDate();
  
    const hollidaysCount = getHollidaysCount(+year, monthNum);
...
function setEaster(year: number){
...
function getHollidaysCount(year: number, monthNum: number){
